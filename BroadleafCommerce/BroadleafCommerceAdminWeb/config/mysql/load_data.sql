...
LOAD DATA INFILE '@@BASE_DIR@@/code_types.txt'
...
FIELDS TERMINATED BY '|' ENCLOSED BY "" 
( DISPLAY_ORDER, PRODUCT_ID, CATEGORY_ID);
...
DELETE FROM broadleafcommerce.BLC_PRODUCT_MEDIA_MAP;

LOAD DATA INFILE '@@BASE_DIR@@/product_media_map.txt' 
INTO TABLE broadleafcommerce.BLC_PRODUCT_MEDIA_MAP
FIELDS TERMINATED BY '|' ENCLOSED BY ""
(BLC_PRODUCT_PRODUCT_ID, MEDIA_ID, MAP_KEY);
...
DELETE FROM broadleafcommerce.BLC_ROLE;

LOAD DATA INFILE '@@BASE_DIR@@/role.txt' 
INTO TABLE broadleafcommerce.BLC_ROLE
FIELDS TERMINATED BY '|' ENCLOSED BY ""
(ROLE_ID, ROLE_NAME);

DELETE FROM broadleafcommerce.BLC_ID_GENERATION;

LOAD DATA INFILE '@@BASE_DIR@@/id_generation.txt' 
INTO TABLE broadleafcommerce.BLC_ID_GENERATION
FIELDS TERMINATED BY '|' ENCLOSED BY ""
(ID_TYPE, BATCH_SIZE, BATCH_START, VERSION);
...
DELETE FROM broadleafcommerce.BLC_CUSTOMER_ROLE;

LOAD DATA INFILE '@@BASE_DIR@@/customer_role.txt' 
INTO TABLE broadleafcommerce.BLC_CUSTOMER_ROLE
FIELDS TERMINATED BY '|' ENCLOSED BY ""
(CUSTOMER_ROLE_ID, ROLE_ID, CUSTOMER_ID);
...
DELETE FROM broadleafcommerce.BLC_CATEGORY_MEDIA_MAP;

LOAD DATA INFILE '@@BASE_DIR@@/category_media_map.txt' 
INTO TABLE broadleafcommerce.BLC_CATEGORY_MEDIA_MAP
FIELDS TERMINATED BY '|' ENCLOSED BY ""
(BLC_CATEGORY_CATEGORY_ID, MEDIA_ID, MAP_KEY);

DELETE FROM broadleafcommerce.BLC_MEDIA;

LOAD DATA INFILE '@@BASE_DIR@@/media.txt' 
INTO TABLE broadleafcommerce.BLC_MEDIA
FIELDS TERMINATED BY '|' ENCLOSED BY ""
(MEDIA_ID, LABEL, NAME, URL);

DELETE FROM broadleafcommerce.BLC_OFFER;

LOAD DATA INFILE '@@BASE_DIR@@/offer.txt' 
...
LOAD DATA INFILE '@@BASE_DIR@@/offer_code.txt' 
...
LOAD DATA INFILE '@@BASE_DIR@@/rating_summary.txt' 
...
LOAD DATA INFILE '@@BASE_DIR@@/review_detail.txt' 
INTO TABLE broadleafcommerce.BLC_REVIEW_DETAIL
FIELDS TERMINATED BY '|' ENCLOSED BY "" 
(REVIEW_DETAIL_ID, HELPFUL_COUNT, NOT_HELPFUL_COUNT, REVIEW_STATUS, REVIEW_TEXT, CUSTOMER_ID, RATING_SUMMARY_ID, RATING_DETAIL_ID)
  SET REVIEW_SUBMITTED_DATE = CURDATE();

DELETE FROM broadleafcommerce.BLC_RATING_DETAIL;

LOAD DATA INFILE '@@BASE_DIR@@/rating_detail.txt' 
...
DELETE FROM broadleafcommerce.BLC_CONTENT;

LOAD DATA INFILE '@@BASE_DIR@@/content.txt' 
INTO TABLE broadleafcommerce.BLC_CONTENT
FIELDS TERMINATED BY '|' ENCLOSED BY "" 
(ID, ACTIVE, CONTENT_TYPE, DISPLAY_RULE, FILE_PATH_NAME, LANGUAGE, MAX_COUNT, priority, SANDBOX, DEPLOYED, NOTE, SUBMITTED_BY)
  SET ACTIVE_END_DATE = DATE_ADD(CURDATE(), INTERVAL 31 DAY),
  ACTIVE_START_DATE = CURDATE(),
  SUBMITTED_DATE = CURDATE();
  
UPDATE broadleafcommerce.BLC_CONTENT
SET SANDBOX = null
WHERE SANDBOX = 'null';

DELETE FROM broadleafcommerce.BLC_CONTENT_DETAILS;

LOAD DATA INFILE '@@BASE_DIR@@/content_details.txt' 
INTO TABLE broadleafcommerce.BLC_CONTENT_DETAILS
FIELDS TERMINATED BY '|' ENCLOSED BY "" 
(ID, CONTENT_HASH, XML_CONTENT);
