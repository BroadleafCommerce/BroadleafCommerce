...
public class OrphanedCategoryListDataSource extends ListGridDataSource {
	...
	protected void executeFetch(final String requestId, DSRequest request, final DSResponse response) {
		...
		service.fetch(ceilingEntityFullyQualifiedClassname, fetchModule.getCto(request), persistencePerspective, null, new EntityServiceAsyncCallback<DynamicResultSet>(EntityOperationType.FETCH, requestId, request, response, this) {
