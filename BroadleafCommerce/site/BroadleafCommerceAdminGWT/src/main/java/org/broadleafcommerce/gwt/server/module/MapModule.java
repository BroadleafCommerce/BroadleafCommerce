...
public class MapModule extends BasicEntityModule {
	...
	public void extractProperties(Map<MergedPropertyType, Map<String, FieldMetadata>> mergedProperties, List<Property> properties) throws NumberFormatException {
		if (mergedProperties.get(MergedPropertyType.MAPSTRUCTUREKEY) != null) {
			extractPropertiesFromMetadata(mergedProperties.get(MergedPropertyType.MAPSTRUCTUREKEY), properties, true);
		}
		if (mergedProperties.get(MergedPropertyType.MAPSTRUCTUREVALUE) != null) {
			extractPropertiesFromMetadata(mergedProperties.get(MergedPropertyType.MAPSTRUCTUREVALUE), properties, true);
		}
	}

	@SuppressWarnings("rawtypes")
	protected Entity[] getMapRecords(Serializable record, MapStructure mapStructure, Map<String, FieldMetadata> valueMergedProperties) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException, SecurityException, IllegalArgumentException, ClassNotFoundException {
		...
		Map map = (Map) PropertyUtils.getProperty(record, mapStructure.getMapProperty());
		Entity[] entities = new Entity[map.size()];
		int j=0;
		for (Object key : map.keySet()) {
			Entity entityItem = new Entity();
			entityItem.setType(record.getClass().getName());
		...
		try {	
			...
			if (mapStructure != null) {
				...
				if (persistentClass == null) {
					...
					keyMergedProperties = getMergedProperties(
				...
				if (persistentClass == null) {
					...
					valueMergedProperties = getMergedProperties(
	...
	public Entity add(String ceilingEntityFullyQualifiedClassname, Entity entity, PersistencePerspective persistencePerspective, String[] customCriteria) throws ServiceException {
		...
		try {
			...
			Serializable instance = dynamicEntityDao.retrieve(Class.forName(entity.getType()), Long.valueOf(entity.findProperty("symbolicId").getValue()));
			Map map = (Map) PropertyUtils.getProperty(instance, mapStructure.getMapProperty());
			...
			return getMapRecords(instance, mapStructure, valueMergedProperties)[0];
		} catch (Exception e) {
	...
	public DynamicResultSet fetch(String ceilingEntityFullyQualifiedClassname, CriteriaTransferObject cto, PersistencePerspective persistencePerspective, String[] customCriteria) throws ServiceException {
		...
		try {
			...
			payload = getMapRecords(records.get(0), mapStructure, valueMergedProperties);
