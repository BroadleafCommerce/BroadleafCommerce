...
public class AdminUserDaoImpl implements AdminUserDao {
    ...
    public void deleteAdminUser(AdminUser user) {
    	if (!em.contains(user)) {
        	user = (AdminUser) em.find(entityConfiguration.lookupEntityClass("org.broadleafcommerce.gwt.server.security.domain.AdminUser"), user.getId());
    	}
        em.remove(user);
    }

    public AdminUser readAdminUserById(Long id) {
        return (AdminUser) em.find(entityConfiguration.lookupEntityClass("org.broadleafcommerce.gwt.server.security.domain.AdminUser"), id);
    }
