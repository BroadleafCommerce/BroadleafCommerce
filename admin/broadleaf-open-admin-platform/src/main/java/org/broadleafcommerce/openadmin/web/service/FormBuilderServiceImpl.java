...
public class FormBuilderServiceImpl implements FormBuilderService {
	...
	public ListGrid buildMainListGrid(Entity[] entities, ClassMetadata cmd)
		...
		for (Property p : cmd.getProperties()) {
			if (p.getMetadata() instanceof BasicFieldMetadata) {
				...
				if (fmd.isProminent() != null && fmd.isProminent()) {
					Field hf = new Field().withName(p.getName())
		...
		return createListGrid(cmd.getCeilingType(), headerFields, type,
				entities);
	}

	@Override
	public ListGrid buildCollectionListGrid(String containingEntityId,
			Entity[] entities, Property field) throws ServiceException,
		...
		// Get the header fields for this list grid. Note that the header fields
		// are different depending on the
		// kind of field this is.
		if (fmd instanceof BasicFieldMetadata) {
			for (Property p : cmd.getProperties()) {
				if (p.getMetadata() instanceof BasicFieldMetadata) {
					BasicFieldMetadata md = (BasicFieldMetadata) p
							.getMetadata();
			...
			if (((BasicCollectionMetadata) fmd).getAddMethodType().equals(
					AddMethodType.PERSIST)) {
				editable = true;
			}
			...
			for (String fieldName : atcmd.getGridVisibleFields()) {
				...
				Field hf = new Field().withName(p.getName()).withFriendlyName(
						p.getMetadata().getFriendlyName());
				headerFields.add(hf);
			}

			type = ListGrid.Type.ADORNED;

			if (((AdornedTargetCollectionMetadata) fmd)
					.getMaintainedAdornedTargetFields().length > 0) {
				editable = true;
			}
			...
			Field hf = new Field().withName(p2.getName()).withFriendlyName(
					p2.getMetadata().getFriendlyName());
			headerFields.add(hf);
			...
			for (Property p : cmd.getProperties()) {
				if (p.getMetadata() instanceof BasicFieldMetadata) {
					...
					if (md.getTargetClass().equals(mmd.getValueClassName())) {
						if (md.isProminent() != null && md.isProminent()) {
							hf = new Field().withName(p.getName())
									.withFriendlyName(md.getFriendlyName());
							headerFields.add(hf);
						}
					}
		...
		ListGrid listGrid = createListGrid(cmd.getCeilingType(), headerFields,
				type, entities);
	...
	protected ListGrid createListGrid(String className,
			List<Field> headerFields, ListGrid.Type type, Entity[] entities) {
		...
		// For each of the entities (rows) in the list grid, we need to build
		// the associated
		// ListGridRecord and set the required fields on the record. These
		...
		for (Entity e : entities) {
			...
			for (Field headerField : headerFields) {
				...
				if (p != null) {
					Field recordField = new Field().withName(
							headerField.getName()).withValue(p.getValue());
	...
	protected void setEntityFormFields(EntityForm ef, List<Property> properties) {
		for (Property property : properties) {
			if (property.getMetadata() instanceof BasicFieldMetadata) {
				BasicFieldMetadata fmd = (BasicFieldMetadata) property
						.getMetadata();

				// Depending on visibility, field for the particular property is
				// not created on the form
				if (!(VisibilityEnum.HIDDEN_ALL.equals(fmd.getVisibility()))
						|| (VisibilityEnum.FORM_HIDDEN.equals(fmd
								.getVisibility()))) {
					String fieldType = fmd.getFieldType() == null ? null : fmd
							.getFieldType().toString();
					// Create the field and set some basic attributes
					Field f;
					if (fieldType
							.equals(SupportedFieldType.BROADLEAF_ENUMERATION
									.toString())) {
						f = new ComboField();
					...
					ef.addField(f, fmd.getGroup(), fmd.getGroupOrder(),
							fmd.getTab(), fmd.getTabOrder());
				}
			}
		}
	}
	...
	public void removeNonApplicableFields(ClassMetadata cmd,
			EntityForm entityForm, String entityType) {
		for (Property p : cmd.getProperties()) {
			if (!ArrayUtils.contains(p.getMetadata().getAvailableToTypes(),
					entityType)) {
	...
	public EntityForm buildEntityForm(ClassMetadata cmd, Entity entity) {
		...
		for (Property p : cmd.getProperties()) {
			if (p.getMetadata() instanceof BasicFieldMetadata) {
				// Check for visibility again, so that the field isn't attempted
				// to be populated with a value on the form
				BasicFieldMetadata bFM = (BasicFieldMetadata) p.getMetadata();
				if ((!(VisibilityEnum.HIDDEN_ALL.equals(bFM.getVisibility())) || (VisibilityEnum.FORM_HIDDEN
						.equals(bFM.getVisibility())))) {
	...
	@Override
	public EntityForm buildEntityForm(ClassMetadata cmd, Entity entity,
			Map<String, Entity[]> collectionRecords) throws ServiceException,
			ApplicationSecurityException {
		...
		for (Property p : cmd.getProperties()) {
			...
			ListGrid listGrid = buildCollectionListGrid(containingEntityId,
					subCollectionEntities, p);
	...
	@Override
	public void populateAdornedEntityFormFields(EntityForm ef, Entity entity,
			AdornedTargetList adornedList) {
		Field field = ef.getFields().get(
				adornedList.getTargetObjectPath() + "."
	...
	public void copyEntityFormValues(EntityForm destinationForm,
		...
		for (Entry<String, Field> entry : sourceForm.getFields().entrySet()) {
			Field destinationField = destinationForm.getFields().get(
					entry.getKey());
			destinationField.setValue(entry.getValue().getValue());
			destinationField
					.setDisplayValue(entry.getValue().getDisplayValue());
		}
	}
	...
	public EntityForm buildAdornedListForm(
			...
			AdornedTargetList adornedList, String parentId)
			throws ServiceException, ApplicationSecurityException {
		EntityForm ef = new EntityForm();
		...
		ClassMetadata collectionMetadata = adminEntityService
				.getClassMetadata(request);

		// We want our entity form to only render the maintained adorned target
		// fields
		List<Property> entityFormProperties = new ArrayList<Property>();
		for (String targetFieldName : adornedMd
				.getMaintainedAdornedTargetFields()) {
		...
		// Add these two additional hidden fields that are required for
		// persistence
		Field f = new Field()
				.withName(
						adornedList.getLinkedObjectPath() + "."
								+ adornedList.getLinkedIdProperty())
		...
		f = new Field().withName(adornedList.getSortField()).withFieldType(
	...
	public EntityForm buildMapForm(MapMetadata mapMd,
			final MapStructure mapStructure, ClassMetadata cmd, String parentId)
			throws ServiceException, ApplicationSecurityException {
		EntityForm ef = new EntityForm();
		...
		keyField.withName("key").withFieldType("combo_field")
		...
		if (mapMd.getKeys() != null) {
			...
			PersistencePackageRequest ppr = PersistencePackageRequest
					.standard().withClassName(
							mapMd.getMapKeyOptionEntityClass());

			Entity[] rows = adminEntityService.getRecords(ppr);

			for (Entity entity : rows) {
				String keyValue = entity.getPMap()
						.get(mapMd.getMapKeyOptionEntityValueField())
						.getValue();
				String keyDisplayValue = entity.getPMap()
						.get(mapMd.getMapKeyOptionEntityDisplayField())
						.getValue();
		...
		List<Property> mapFormProperties = new ArrayList<Property>(
				Arrays.asList(cmd.getProperties()));
		CollectionUtils.filter(mapFormProperties, new Predicate() {
			@Override
			public boolean evaluate(Object object) {
				Property p = (Property) object;
				return ArrayUtils.contains(p.getMetadata()
						.getAvailableToTypes(), mapStructure
						.getValueClassName());
			}
		});
		...
		Field f = new Field().withName("symbolicId")
				.withFieldType(SupportedFieldType.HIDDEN.toString())
				.withValue(parentId);
		ef.addHiddenField(f);

		f = new Field().withName("priorKey").withFieldType(
				SupportedFieldType.HIDDEN.toString());
		ef.addHiddenField(f);
