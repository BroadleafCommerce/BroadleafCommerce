 ...
 *
 * The intention of these method implementations are to make a Server to Server API call.
 ...
 *
 * Note: in the case where a gateway doesn't support confirming the transaction before it is submitted
 * (i.e. paymentGatewayConfigurationService.handlesTransactionConfirmation() == false)
 * The PaymentGatewayWebResponseService will handle translation of the final transaction response from the gateway.
 * There is no need to re-call this service if the gateway doesn't support confirming the transaction.
 *
 */
