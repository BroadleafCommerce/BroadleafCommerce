...
public class OptimisticLockUtils {
     ...
     */
    public static <T> T performOptimisticLockUpdate(
            String name,
    ...
    ) throws OptimisticLockMaxRetryException, OptimisticLockInvalidStateException {
        int retryCount = 0;
        boolean saveSuccessful = false;
        T entity = null;
        while (!saveSuccessful) {
            if (retryCount >= maxRetryCount) {
                log.debug("Max retry count was reached while trying to perform " + name + " on "
                        + entityClass.getSimpleName() + " with id: " + identifier);
                throw new OptimisticLockMaxRetryException("Unable to perform " + name + " on "
                        + entityClass.getSimpleName() + " with id: " + identifier + ". " + "Tried " + retryCount
                        + " times, but the version for this entity continues to be concurrently modified.");
            }
            try {
                entity = doTransactionalOptimisticUpdate(
                        name, operation, entityClass, identifier, transactionManager, entityManager
                );
                saveSuccessful = true;
                log.debug(name + " for " + entityClass.getSimpleName() + " with ID: " + identifier + " performed "
                        + retryCount + " retries.");

            } catch (OptimisticLockException e) {
                log.debug("Optimistic locking failure. Concurrent modification detected when attempting to modify "
                        + entityClass.getSimpleName() + " with id: " + identifier);
            }
            retryCount++;
        }
    ...
    protected static <T> T doTransactionalOptimisticUpdate(
            String name,
            UpdateOperation<T> operation,
            Class<? extends T> entityClass,
            Object identifier,
        ...
        try {
            ...
            if (operation instanceof ValidatedUpdateOperation
                    && !((ValidatedUpdateOperation<T>) operation).isValid(entity)) {
                log.debug("Entity state was found to be invalid while trying to perform " + name + " on "
                        + entityClass.getSimpleName() + " with id: " + identifier);
                throw new OptimisticLockInvalidStateException("Unable to perform " + name + " on "
                        + entityClass.getSimpleName() + " with id: " + identifier
                        + ". Aborting update due to invalid state.");
            }
