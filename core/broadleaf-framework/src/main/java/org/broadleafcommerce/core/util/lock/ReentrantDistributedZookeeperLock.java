...
public class ReentrantDistributedZookeeperLock implements DistributedLock {
     ...
     */
    private static final Set<String> INITIALIZED_LOCK_PATHS = Collections.synchronizedSet(new HashSet<>());
        ...
        try {
            ...
            final String localLockPath = GenericOperationUtil.executeRetryableOperation(new GenericOperation<String>() {
                ...
                public String execute() throws Exception {
                    return getZookeeperClient().create(
                            getLockFolderPath() + '/' + getFullLockName(),
                            null,
                            getAcls(),
            ...
            synchronized (LOCK_MONITOR) {
                ...
                while (true) {
                    ...
                    if (firstLocked != null && localLockPath.endsWith(firstLocked)) {
                        ...
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("ZK lock was acquired by " + Thread.currentThread().getName()
                                    + ". The lock name is " + currentlockPath);
                        }
            ...
            GenericOperationUtil.executeRetryableOperation(new GenericOperation<Void>() {
                ...
                public Void execute() throws Exception {
                    ZookeeperUtil.makePath(
                            getLockFolderPath(), null, getZookeeperClient(), CreateMode.PERSISTENT, acls
                    );
                    return null;
                }
            }, getFailureRetries(), getRetryWaitTime(), isAdditiveWaitTtimes(), null);
    ...
    public boolean canParticipate() {
        if (getEnvironment() != null) {
            ...
            boolean lockNameParticipation = getEnvironment().getProperty(
                    getLockAccessPropertyName(), Boolean.class, true
            );
            if (lockNameParticipation) {
                //Now, check the global access in this environment.
                return getEnvironment().getProperty(
                        DistributedLock.GLOBAL_ENV_CAN_OBTAIN_LOCK_PROPERTY_NAME, Boolean.class, true
                );
            } else {
                return false;
            }
