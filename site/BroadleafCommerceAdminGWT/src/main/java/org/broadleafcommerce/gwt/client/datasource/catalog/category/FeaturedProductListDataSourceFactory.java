...
public class FeaturedProductListDataSourceFactory {
	...
	public static void createDataSource(String name, AsyncCallback<DataSource> cb) {
		if (dataSource == null) {
			...
			persistencePerspective.addPersistencePerspectiveItem(PersistencePerspectiveItemType.JOINSTRUCTURE, new JoinStructure(symbolName, linkedObjectPath, linkedIdProperty, targetObjectPath, targetIdProperty, EntityImplementations.FEATUREDPRODUCT, sortField, true));
			...
			dataSource = new ListGridDataSource(CeilingEntities.PRODUCT, name, persistencePerspective, AppServices.DYNAMIC_ENTITY, modules);
